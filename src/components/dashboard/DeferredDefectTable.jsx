...
import { getDeferredDefects, updateDeferralComment } from '../../services/DefectService.js'
import { useAuth } from '../../contexts/AuthContext.jsx'
import { useFilters } from '../../contexts/FilterContext.jsx'
import { formatDate, formatNumber, truncateText } from '../../utils/formatters.js'

export default function DeferredDefectTable() {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true)
    try {
      const result = await getDeferredDefects(filters)
      setData(Array.isArray(result) ? result : [])
    } catch (err) {
      console.error('[DeferredDefectTable] Error fetching deferred defects:', err)
      setData([])
    } finally {
      setIsLoading(false)
    }
  ...
  const handleDeferralCommentUpdate = useCallback(
    async (row, newValue) => {
      if (!canEditComments) return

      const userId = currentUser ? currentUser.id : null

      const result = await updateDeferralComment(row.issueId, newValue, userId)

      if (result.success) {
        setData((prev) =>
          prev.map((item) =>
            item.issueId === row.issueId
              ? { ...item, deferralComment: newValue }
              : item
          )
        )
      } else {
        console.error('[DeferredDefectTable] Failed to update deferral comment:', result.error)
      }
    },
    [canEditComments, currentUser]
  )
      ...
      },
      {
        ...
        render: (value, row) => (
          <EditableCell
            ...
            type="text"
            onSave={(newValue) => handleDeferralCommentUpdate(row, newValue)}
            placeholder="N/A"
            ariaLabel="Deferral Comment"
            maxLength={500}
          />
        ),
      },
    ]
  }, [canEditComments, handleDeferralCommentUpdate])

  if (isLoading) {
    return <LoadingSpinner size="md" message="Loading deferred defects..." />
  }

  if (!data || data.length === 0) {
    return (
      <EmptyState
        title="No Deferred Defects"
        message="No deferred defects found for the selected filters. Try adjusting your filter criteria."
      />
    )
  }
  ...
  return (
    <div className="animate-fade-in">
      <DataTable
        ...
        pageSize={10}
        sortable={true}
        emptyMessage="No deferred defects available"
      />
    </div>
  )
}